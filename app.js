...
app.post('/login', async (req, res) => {
  try {
    ...
    if (!ownerName || !password) {
      return res.status(400).json({ error: "Owner name and password are required" });
    }
...
app.post('/login-user', async (req, res) => {
  try {
    ...
    // Get all shops for displaying products
    const shops = await ownerModel.find({});
    res.status(200).json({ message: "Login successful", user, shops });
  } catch (err) {
